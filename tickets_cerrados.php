...
$orden_map = [
    ...
    'titulo' => 't.titulo',
    'estado' => 't.estado',
    'fecha' => 't.fecha_ultima_modificacion',
    'solicitante' => 't.nombre_solicitante',
    'responsable' => 't.responsable'
];
$columna_orden = $orden_map[$orden];

// Obtener conteos por estado (abiertos)
$conteos_estado = [];
...
try {
    // Contar total de tickets cerrados
    $where = "es_cerrado = 1";
    $params = [];
...
<body>
    ...
    <div class="container mt-5">
        ...
        <div class="mb-3">
            <a href="tickets.php" class="btn btn-outline-primary">Todos</a>
            <a href="tickets.php?estado=sin%20abrir<?php echo !empty($busqueda) ? '&buscar=' . urlencode($busqueda) : ''; ?>" class="btn btn-outline-secondary">Sin abrir (<?php echo $conteos_estado['sin abrir']; ?>)</a>
            <a href="tickets.php?estado=en%20conocimiento<?php echo !empty($busqueda) ? '&buscar=' . urlencode($busqueda) : ''; ?>" class="btn btn-outline-info">En conocimiento (<?php echo $conteos_estado['en conocimiento']; ?>)</a>
            <a href="tickets.php?estado=en%20proceso<?php echo !empty($busqueda) ? '&buscar=' . urlencode($busqueda) : ''; ?>" class="btn btn-outline-warning">En proceso (<?php echo $conteos_estado['en proceso']; ?>)</a>
            <a href="tickets.php?estado=pendiente%20de%20cierre<?php echo !empty($busqueda) ? '&buscar=' . urlencode($busqueda) : ''; ?>" class="btn btn-outline-danger">Pendiente de cierre (<?php echo $conteos_estado['pendiente de cierre']; ?>)</a>
            <a href="tickets_cerrados.php" class="btn btn-success">Tickets Cerrados (<?php echo $conteo_cerrados; ?>)</a>
        ...
        <?php if (empty($tickets)): ?>
            <div class="alert alert-info">
                <?php echo !empty($busqueda) ? "No se encontraron tickets cerrados con esa búsqueda" : "No hay tickets cerrados"; ?>
            </div>
        <?php else: ?>
            <!-- Vista Lista (tabla) -->
                    ...
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th><a href="?orden=fecha&dir=<?php echo $orden === 'fecha' && $direccion === 'ASC' ? 'DESC' : 'ASC'; ?><?php echo !empty($busqueda) ? '&buscar=' . urlencode($busqueda) : ''; ?>&pagina=1" style="text-decoration: none; color: inherit;">Fecha Cierre <?php echo $orden === 'fecha' ? ($direccion === 'ASC' ? '▲' : '▼') : ''; ?></a></th>
                                <th>Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr class="ticket-list-row">
                                ...
                                <td><?php echo formatearFecha($ticket["fecha_ultima_modificacion"]); ?></td>
                                <td>
        ...
        <nav aria-label="Paginación" class="mt-4">
            <ul class="pagination justify-content-center">
                ...
                <li class="page-item">
                    <a class="page-link" href="?pagina=<?php echo $total_paginas; ?><?php echo !empty($busqueda) ? '&buscar=' . urlencode($busqueda) : ''; ?>">Ultima</a>
                </li>
                <?php endif; ?>
            </ul>
        </nav>
